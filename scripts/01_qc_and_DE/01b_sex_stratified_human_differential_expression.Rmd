...
metadata <- subset(metadata, sex == "M")
```
...
```{r}
base_path <- "../../results"
results_folder <- "human_kidney_sex_stratified" #
...
saveToPDF(paste0("../../results/", results_folder, "/MDS/MDS_raw_",tissue, sex,".pdf"), width = 10, height = 6)
```
...
write.table(lcpm_gene_names, paste0("../../results/", results_folder, "/counts/filtered_lcpm_gene_names_", tissue, sex,".txt"), sep= "\t", row.names = FALSE, quote=FALSE)
...
write.table(cpm_gene_names, paste0("../../results/", results_folder, "/counts/filtered_cpm_gene_names_", tissue, sex,".txt"), sep= "\t", row.names = FALSE, quote=FALSE)
```
...
saveToPDF(paste0("../../results/", results_folder, "/MDS/MDS_filtered_", tissue, sex,".pdf"), width = 6, height = 6)
```
...
# save
path <- (paste0("../../results/", results_folder, "/library/gene_density_", tissue, sex))
...
path <- (paste0("../../results/", results_folder, "/library/lcpm_boxplots_", tissue, sex))
...
path <- paste0(paste0("../../results/", results_folder, "/voom/voom_raw_", tissue, sex))
...
```{r varpart}
form_varPart <- ~ (1|group) + PMI + Expired.Age + Last.BMI.Score + (1|lane)
...
saveToPDF(paste0("../../results/", results_folder, "/variance/variance_partition_violins_", tissue, sex,".pdf"), width = 8.5, height = 5)
...
write.table(variance_explained, paste0("../../results/", results_folder, "/variance/variance_explained_", tissue, sex,".tsv"),sep = "\t", quote = FALSE, row.names = FALSE)
```
...
plotCorrMatrix( C )
saveToPDF(paste0("../../results/", results_folder, "/variance/CCA_with_project_", tissue, sex,".pdf"), width = 8, height = 8)
...
design <- model.matrix( ~ 0 + dge.filtered.norm$samples$group + dge.filtered.norm$samples$PMI + dge.filtered.norm$samples$Expired.Age + dge.filtered.norm$samples$Last.BMI.Score + dge.filtered.norm$samples$lane)
colnames(design) <- c("control", "sepsis", "PMI", "Age", "BMI","lane")
# Inspect
design
```
...
sumTable
write.table(sumTable, paste0("../../results/", results_folder, "/DEGs/DEGs_summary_q", qval,"_log2FC", lfc.cutoff, "_", tissue, sex,".txt"), quote = FALSE, sep = "\t")
...
for (i in allComparisons) {
  vTopTableAll <- topTable(
    ...
    saveRDS(vTopTableAll, file = 
            paste0("../../rObjects/gene_tables/", i,"_gene_table_", tissue, sex,".rds"))
  path <- paste0("../../results/", results_folder, "/DEGs/", i, "_q1.00_", tissue, sex,".txt", sep = "") 
  ...
  vTopTableSig <-
    topTable( 
      ...
      genelist = genes_relevant, 
      confint = TRUE)
  path <- paste0("../../results/", results_folder, "/DEGs/", i, "_q", qval, "_lfc", lfc.cutoff,"_", tissue, sex,".txt", sep = "") 
...
ggplot(data = pcaWithMetadata, aes(x = PC1, y = PC2, shape = group, color = group)) +
  ...
  theme_bw() 
saveToPDF(paste0("../../results/", results_folder, "/PCA/PCA_dim1&2", sex, ".pdf"), width = 8, height = 5)

ggplot(data = pcaWithMetadata, aes(x = PC2, y = PC3, shape = group, color = group)) +
  geom_point(size = 2.5) +
  geom_text(aes(label = Sample_ID), hjust = -0.1, vjust = 0.5, size = 3) + # Added layer
  theme_bw() 
saveToPDF(paste0("../../results/", results_folder, "/PCA/PCA_dim2&3", sex, ".pdf"), width = 8, height = 5)
...
for (i in allComparisons) {
  group1_vs_group2 <-
    read.delim(paste0("../../results/", results_folder, "/DEGs/", i,"_q1.00_", tissue, sex,".txt"))
    i <- gsub("_", " ", i)
    ...
    i <- gsub(" ", "_", i)
    pdf(paste0("../../results/", results_folder, "/volcano/", i, sex,".pdf"), height = 6, width = 6)
    print(p)
    dev.off()
  } else {
  color_values <- vector()
  ...
  if (num != 0) {
    ...
    i <- gsub(" ", "_", i)
    # save
    path <-paste0("../../results/", results_folder, "/volcano/",i, sex)
    pdf(paste0(path, ".pdf"),
        height = 8,
        width = 8)
    print(p)
...
for (i in allComparisons) {
  filepath <- paste0("../../results/", results_folder, "/DEGs/", i,"_q1.00_", tissue, sex,".txt")
  assign(paste0(i),
...
write.xlsx(list_of_datasets, file = paste0("../../results/", results_folder, "/DEGs/DEGs.q1.00_", tissue, sex,".xlsx"))
```
