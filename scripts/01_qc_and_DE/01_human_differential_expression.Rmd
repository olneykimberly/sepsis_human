...
```{r varpart}
form_varPart <- ~ (1|group) +  (1|sex) + PMI + Expired.Age + Last.BMI.Score + RIN + (1|lane)
...
sorted_info$RIN <- as.numeric(sorted_info$RIN)
```
...
```{r}
form <- ~ group + PMI + Expired.Age + Last.BMI.Score + lane + RIN
...
# Save
```{r}
write.table(sorted_info, paste0("../../rObjects/", projectID, "_sorted_info.txt"), sep = "\t")
